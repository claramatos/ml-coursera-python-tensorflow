...
def linear_reg_cost_function(X, y, theta, lambda_=0.0, as_tensor=False):
    """
...
def optimize(linear_reg_cost_function, theta, optimizer, options):
    for j in range(options['maxiter']):
        loss_val, grad_val = linear_reg_cost_function(theta)
        gradients = [grad_val]
...
def train_linear_reg(linear_reg_cost_function, X, y, lambda_=0.0, maxiter=200):
    ...
    initial_theta = tf.Variable(initial_theta, shape=(initial_theta.shape[0],))
    optimizer = tf.keras.optimizers.Adam(learning_rate=0.5)
    theta = optimize(cost_function, initial_theta, optimizer, options)

...
def nn_cost_function(nn_params,
    ...
    a1, a2, a3, z3 = forward_propagation(theta1, theta2, X)
    J = tf.nn.sigmoid_cross_entropy_with_logits(logits=z3, labels=y_matrix, name='loss')
    J = tf.reduce_mean(tf.reduce_sum(J, axis=1), name='loss')
    ...
    J += (lambda_ / (2 * m)) * (tf.reduce_sum(tf.square(theta1_reg)) + tf.reduce_sum(tf.square(theta2_reg)))

    # backpropagation
    delta_3 = a3 - y_matrix
    z2 = hypothesis(a1, theta1.T)
    delta_2 = hypothesis(delta_3, theta2)[:, 1:] * sigmoid_gradient(z2)

    Delta_1 = hypothesis(tf.transpose(delta_2), a1)
    Delta_2 = hypothesis(tf.transpose(delta_3), a2)

    theta1_grad = (1 / m) * Delta_1.numpy()
    theta1_grad[:, 1:] += (lambda_ / m) * theta1[:, 1:]
    theta2_grad = (1 / m) * Delta_2.numpy()
...
def predict(Theta1, Theta2, X):
    ...
    _, _, a3, _ = forward_propagation(Theta1, Theta2, X)
...
def debug_initialize_weights(fan_out, fan_in):
    ...
    return W.reshape(fan_out, 1 + fan_in, order='F')
